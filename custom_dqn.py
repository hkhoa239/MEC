...
def simulate():
    ...
    checkpoint_path = "train_dqn/2025-02-10T18-47-49/mec_net_6.chkpt"
...
def evaluate(env, mec, episodes=10):
    total_rewards = []
    ...
    for episode in range(episodes):
        ...
        print(state)
        while True:
            state = torch.tensor(state, device=mec.device).unsqueeze(0)
            for action in range(6):
                with torch.no_grad():
                    # action_values = mec.net(state, model="online")
...
def main():
    ...
    mec = MECAgent(state_dim=env.observation_space.shape, action_dim=env.action_space.n, save_dir="", checkpoint=checkpoint_path)
    evaluate(env, mec, episodes=1)
